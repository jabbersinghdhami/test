<?php
namespace Piquant\MikadofModules\Header\Lib;
/**
...
class HeaderConnector {
    ...
    public function connect($config = array()) {
        do_action('piquant_mikado_pre_header_connect');
        ...
        if(!empty($config['affect_content'])) {
            add_filter('piquant_mikado_content_elem_style_attr', array($this, 'contentMarginFilter'));
        }

        if(!empty($config['affect_title'])) {
            add_filter('piquant_mikado_title_content_padding', array($this, 'titlePaddingFilter'));
        }

        if(!empty($config['affect_slider'])) {
            add_filter('piquant_mikado_slider_item_padding', array($this, 'sliderItemPaddingFilter'));
            add_filter('piquant_mikado_slider_navigation_padding', array($this, 'sliderNavigationPaddingFilter'));
        }

        do_action('piquant_mikado_after_header_connect');
    }
     ...
     *
     * Hooks to piquant_mikado_title_content_padding filter
     *
     * @return int
     */
     ...
     * Returns padding value calculated from header height.
     *
     * Hooks to piquant_mikado_slider_item_padding filter
     *
     * @return int
     */
