...
namespace Piquant\MikadofModules\Shortcodes\TabbedGallery;
...
class TabbedGalleryItem implements ShortcodeInterface {
	...
	public function vcMap() {
		vc_map(array(
			...
			'params'                  => array_merge(
				piquant_mikado_icon_collections()->getVCParamsArray(),
				array(
					array(
						'type'        => 'textfield',
						'admin_label' => true,
	...
	public function render($atts, $content = null) {
		...
		return piquant_mikado_get_shortcode_module_template_part('templates/tabbed-gallery-item-template', 'tabbed-gallery', '', $params);
	}
	...
	private function getTabData($params) {
		...
		if($params['icon'] !== '') {
			...
			if($params['icon'] !== '') {
				$data['data-icon-html'] = piquant_mikado_icon_collections()->renderIcon($params['icon'], $params['icon_pack']);
			}
		}
