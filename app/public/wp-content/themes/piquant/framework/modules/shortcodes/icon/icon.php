<?php
namespace Piquant\MikadofModules\Shortcodes\Icon;

use Piquant\MikadofModules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class Icon
 * @package Piquant\MikadofModules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface {
    ...
    public function vcMap() {
        vc_map(array(
            ...
            'params'                    => array_merge(
                \PiquantMikadoIconCollections::get_instance()->getVCParamsArray(),
                array(
                    array(
                        'type' => 'attach_image',
                        'heading' => 'Image',
                    ...
                    array(
                        ...
                        'description' => esc_html__('Please insert border radius(Rounded corners) in px. For example: 4 ', 'piquant'),
    ...
    public function render($atts, $content = null) {
        ...
        $html = piquant_mikado_get_shortcode_module_template_part('templates/icon', 'icon', '', $params);
        ...
        if(($params['type'] !== 'normal' && !empty($params['shape_size'])) ||
           ($params['type'] == 'normal')
            ...
            if(!empty($params['custom_size'])) {
                $iconStyles[] = 'font-size:'.piquant_mikado_filter_px($params['custom_size']).'px';
            }
        }
    ...
    private function generateIconHolderStyles($params) {
        ...
        if(isset($params['type']) && $params['type'] !== 'normal') {
            ...
            if(!empty($shapeSize)) {
                $iconHolderStyles[] = 'width: '.piquant_mikado_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'height: '.piquant_mikado_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'line-height: '.piquant_mikado_filter_px($shapeSize).'px';
            }
            ...
            if(isset($params['border_width']) && $params['border_width'] !== '') {
                $iconHolderStyles[] = 'border-width: '.piquant_mikado_filter_px($params['border_width']).'px';
            ...
            if($params['type'] == 'square') {
                if(isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: '.piquant_mikado_filter_px($params['border_radius']).'px';
                }
            }
        }
